      ...
      <div class="card-container">
        <vs-card v-bind:style="{ margin: '6px 6px' }">
          ...
          <template #interactions>
            <vs-button
              circle
              href="https://github.com/0xe82de"
              blank
              class="white icon"
            >
              <i class="bx"
                ><img src="img/github.png" width="15px" height="15px"
              /></i>
            </vs-button>
          </template>
        </vs-card>
        ...
        <vs-card v-bind:style="{ margin: '6px 6px' }">
          ...
          <template #interactions>
            <vs-button
              circle
              href="https://github.com/zlzzlzz2l"
              blank
              class="white icon"
            >
              <i class="bx"
        ...
        <vs-card v-bind:style="{ margin: '6px 6px' }">
          ...
          <template #interactions>
            <vs-button
              circle
              href="https://github.com/tkdrms568"
              blank
              class="white icon"
            >
              <i class="bx"
  ...
  watch: {
    current_page() {
      ...
      if (this.notice_list_len < this.maxNoticeView + 1) {
        ...
        // 보여줄 페이지의 갯수를 가져옴
        if (this.maxNoticeView * this.current_page > this.notice_list_len)
          maxCnt = this.notice_list_len % this.maxNoticeView;
        // maxNoticeView * page가 len과 같은 경우 maxNoticeView만큼 가져옴
        else maxCnt = this.maxNoticeView;
        for (let i = 0; i < maxCnt; i++) {
          this.current_page_notice_list.push(
            this.notice_list[(this.current_page - 1) * this.maxNoticeView + i]
          );
        }
      }
    }
    ...
    getNotice() {
      http
        ...
        .then((response) => {
          ...
          if (this.notice_list_len % this.maxNoticeView === 0)
            this.page_length = this.notice_list_len / this.maxNoticeView;
          else
            this.page_length =
              parseInt(this.notice_list_len / this.maxNoticeView) + 1;
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            this.add_notice.title =
              document.getElementById("input-title").value;
            this.add_notice.content =
